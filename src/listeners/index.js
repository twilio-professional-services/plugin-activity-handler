...
// The activities in this array are used by the agent or programmatically
// to indicate the agent is having an issue
const issueActivities = [
	Activity.systemIssue,
	Activity.sysG,
	Activity.sysV
];
...
const validateAndSetWorkerActivity = () => {
  ...
  const { workerActivitySid, workerActivityName } = WorkerState;
		...
		&& !FlexState.hasWrappingTask)
	{
		...
		const loginActivitySid = manager.serviceConfiguration?.ui_attributes?.activityHandlerPlugin?.loginActivitySid;
		const loginActivity = loginActivitySid && FlexState.getActivityBySid(loginActivitySid);
		// If a login activity SID is defined on Flex configuration and that SID matches
		// an activity on this Flex account, moving worker to it
		if (loginActivity) {
			console.log(`Setting worker from "${workerActivityName}" to "${loginActivity.name}" on initial login`);
			setWorkerActivity(loginActivity.sid);
		}
	}
}
...
Actions.addListener('beforeSetActivity', (payload, abortFunction) => {
  ...
  else if (FlexState.hasActiveTask || FlexState.hasWrappingTask) {
    abortFunction();
...
Actions.addListener('beforeCompleteTask', async (payload,abortFunction) => {
	const pendingActivity = FlexState.pendingActivity;
	// Setting worker to the pending activity only when all chats are completed
	console.log("beforeCompleteTask FlexState.hasLiveChatTask", FlexState.hasLiveChatTask, "FlexState.hasWrappingTask" , FlexState.hasWrappingTask);

	if(!FlexState.hasLiveChatTask && !FlexState.hasWrappingTask){
		if (pendingActivity) {
...
const handleReservationEnded = async (reservation, eventType) => {
	...
	if (issueActivities.includes(WorkerState.workerActivityName)) {
		// If the agent's current activity indicates they are having an issue,
		// we will leave them in that activity and clear any pending activity
		// change. This is mainly to prevent the agent from being automatically
		// changed to  Available when they're having an issue of some kind.
		FlexState.clearPendingActivityChange();
		return;
	}
